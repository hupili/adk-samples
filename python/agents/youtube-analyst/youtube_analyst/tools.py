...
def aggregate_comment_sentiment(
    video_ids: list, comments_per_video: int = 10
) -> dict:
    """
    ...
    for vid in video_ids:
        ...
        aggregated_data[vid] = [c["text"] for c in comments_data]
...
def get_video_transcript(video_id: str) -> str:
    ...
    try:
        ...
        for entry in transcript:
            # Convert seconds to MM:SS. Handle both dict (old API/mocks) and object (new API)
            start_val = (
                entry["start"] if isinstance(entry, dict) else entry.start
            )
...
def list_skills() -> str:
    """
    Lists the specialized skills/workflows available to the agent.
    Call this when you are unsure how to fulfill a complex user request to see what workflows are available.
    """
    return """
    Available Skills:
    1. 'kol_discovery': Finding and ranking influencers based on engagement metrics.
    2. 'sentiment_analysis': Analyzing audience mood from video comments.
    3. 'deep_exploration': (ATTENTION GUARDIAN) Finding specific timestamps across multiple videos on a topic to save user time.
    4. 'daily_briefing': (ATTENTION GUARDIAN) Providing a time and location-specific news briefing based on recent videos.
    5. 'debate_synthesizer': (ATTENTION GUARDIAN) Extracting community consensus and arguments from highly replied-to comment threads.
    6. 'creative_insight_analyzer': (B2B/CREATIVE) Using multimodal vision to deconstruct a video's visual elements, hooks, and editing rhythm.
    7. 'abcd_framework_audit': (B2B/CREATIVE) Strictly evaluates a video against Google's ABCD ad effectiveness framework.
    8. 'product_launch_audit': (B2B/SELLER) Provides an Executive Dashboard comparing Creator vs Audience verdicts for a recent client product launch.
    9. 'industry_landscape_briefing': (B2B/SELLER) Synthesizes macro trends and competitor actions using trending data and specific analyst channels.
    10. 'poi_discovery_briefing': (B2B/TRAVEL) Extracts specific local activities, sentiments, and timestamp links from travel vlogs into a shareable HTML BD report.
    11. 'visualization_reporting': Generating charts and publishing final HTML reports.
    12. 'multi_video_synthesis': (ATTENTION GUARDIAN) Mapping semantic vectors across 10+ videos simultaneously to extract absolute truth and consensus.
    
    Use the 'load_skill' tool with the exact name to load the detailed workflow.
    """


def load_skill(skill_name: str) -> str:
    """
    Loads the detailed workflow instructions for a specific skill.

    Args:
        skill_name: The exact name of the skill (e.g., 'kol_discovery').
    """
    try:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        skill_path = os.path.join(base_dir, "skills", f"{skill_name}.txt")
